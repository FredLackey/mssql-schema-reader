...
var retainFile = function (filepath, callback) {
    ...
    fs.readFile(filepath, 'utf8', function (err, data) {
        ...
        var date    = new Date(doc.created),
            ...
            newPath = path.join(__dirname, newName);
...
var retainFileOptional = function (filepath, keepOldVersion, callback) {
    if (!!keepOldVersion) {
...
var fetchWithSave = function (config, filePath, keepOldVersion, callback) {
    retainFileOptional(filePath, keepOldVersion, function (err) {
        if (err) { return callback (err); }
